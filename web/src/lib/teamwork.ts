...
//
// Pass skipTasks=true to skip pulling the previous month's tasks and
// logged hours — useful when the user wants to start fresh from a
// spreadsheet rather than templating from last month. Reference data
// (projects, tasklists, users) is still fetched because the
// spreadsheet importer needs it to resolve names to Teamwork IDs.
export async function loadMonth(month: string, skipTasks = false): Promise<LoadedMonth> {
  ...
  // 2) Tasks for the date window with the magic flags. Skipped when
  // the user wants reference data only.
  ...
  if (!skipTasks) {
    ...
    // skipTasks=true — still need to fetch users separately because
    // they would normally arrive sideloaded with the tasks fetch.
    try {
      const uData = await callWorker<any>("/projects/api/v3/people.json", {
        params: { pageSize: "200" },
      });
      const people: any[] = uData.people || uData.users || [];
      ...
      // non-fatal — user can still manually pick assignees in the import UI
    }
  }
  ...
  // Skipped along with tasks — only useful when comparing planned vs logged.
