...
import { lock as lockApp } from "../lib/config";
...
export default function Planner({ onOpenSettings }: { onOpenSettings: () => void }) {
  ...
  // Tracks where the current grid data came from, so the subtitle stays accurate.
  // null = nothing loaded yet, "teamwork" = templated from previous month,
  // "spreadsheet" = imported from CSV/XLSX/Google Sheets.
  const [loadSource, setLoadSource] = useState<"teamwork" | "spreadsheet" | null>(null);
  ...
  function load(skipTasks = false) {
    ...
    if (!skipTasks) setLoadSource("teamwork");

    loadMonth(sourceMonth, skipTasks)
      .then((loaded) => {
        setData(loaded);

        if (skipTasks) {
          // Start-fresh path — empty grid, ready for spreadsheet import
          setRows([]);
          return;
        }
  ...
  return (
    <div className="min-h-screen pb-32">
      <Header
        ...
        onLoad={() => setSourcePickerOpen(true)}
        loading={loading}
      ...
      {importOpen && data && (
        <ImportDialog
          ...
          users={data.users}
          defaultStart={monthStartEnd(targetMonth).start}
          defaultEnd={monthStartEnd(targetMonth).end}
          onCancel={() => setImportOpen(false)}
          onImport={(imported, mode) => {
            const newRows: EditableTask[] = imported.map((r, i) => ({
              rowId: `imp-${Date.now()}-${i}-${Math.random().toString(36).slice(2, 6)}`,
              sourceTaskId: null,
            ...
            // If we're replacing or there was no previous load, the grid is now
            // spreadsheet-sourced. If merging with an existing Teamwork load,
            // keep the original source label.
            if (mode === "replace" || loadSource == null) {
              setLoadSource("spreadsheet");
            }
            setImportOpen(false);
          }}
        />
      ...
      {sourcePickerOpen && (
        <SourcePickerDialog
          ...
          onPickSpreadsheet={() => {
            setSourcePickerOpen(false);
            // Load reference data first (if not already loaded), then open import
            if (data) {
              setImportOpen(true);
            } else {
              setLoading(true);
              setLoadError(null);
              loadMonth(sourceMonth, true)
                .then((loaded) => {
                  setData(loaded);
                  setRows([]);
                  setImportOpen(true);
...
function Header({
  sourceMonth, targetMonth, unlinkMonths, loadSource,
  setSourceMonth, setTargetMonth, setUnlinkMonths,
  onLoad, loading, onOpenSettings, loaded,
}: {
  sourceMonth: string;
  targetMonth: string;
  unlinkMonths: boolean;
  loadSource: "teamwork" | "spreadsheet" | null;
      ...
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">
            {loadSource === "spreadsheet" ? (
              <>Planning <strong>{monthLabel(targetMonth)}</strong> from spreadsheet</>
            ) : loadSource === "teamwork" ? (
              <>Planning <strong>{monthLabel(targetMonth)}</strong> using <strong>{monthLabel(sourceMonth)}</strong> as template</>
            ) : (
              <>Planning <strong>{monthLabel(targetMonth)}</strong></>
            )}
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-2">
            <label className="text-xs text-zinc-600">
              {loadSource === "spreadsheet" ? "Template:" : "Source:"}
            </label>
            <input
              type="month"
              value={sourceMonth}
              ...
              title={loadSource === "spreadsheet"
                ? "Only used as a fallback for any rows missing dates"
                : "Month to use as a template"}
            />
          </div>
          {unlinkMonths && (
            <div className="flex items-center gap-2">
          ...
          <button
            ...
            title="Pick where to load tasks from"
          >
          ...
          <button
            onClick={() => lockApp()}
            className="px-3 py-1.5 text-sm border border-zinc-300 rounded-md hover:bg-zinc-50"
            title="Clear the decrypted key from memory"
          >
            🔒 Lock
          </button>
        </div>
...
function ProjectCard({
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-lg">
      <button
        onClick={onToggle}
        className="w-full px-4 py-3 flex items-center justify-between hover:bg-zinc-50 text-left rounded-t-lg"
      >
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute z-50 mt-1 w-64 bg-white border border-zinc-200 rounded-md shadow-lg max-h-72 overflow-y-auto">
            {users.length === 0 && (
...
function SourcePickerDialog({
  sourceMonth, loaded, onCancel, onPickPreviousMonth, onPickSpreadsheet,
}: {
  sourceMonth: string;
  loaded: boolean;
  onCancel: () => void;
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div className="px-6 py-4 border-b border-zinc-200">
          <h2 className="text-lg font-semibold">
            {loaded ? "Reload tasks" : "Load tasks"}
          </h2>
          <p className="text-sm text-zinc-600 mt-1">
            Where do you want to load tasks from?
          </p>
        </div>
        ...
        <div className="px-3 py-3 space-y-1">
          <button
            ...
            className="w-full px-4 py-3 text-left rounded-md hover:bg-blue-50 border border-zinc-200"
          >
            <div className="font-medium text-sm flex items-center gap-2">
              <span>📋</span> Use {monthLabel(sourceMonth)} as template
            </div>
            <div className="text-xs text-zinc-600 mt-1">
              Pulls last month's tasks from Teamwork. Dates shift forward, edit before pushing.
            </div>
          </button>
          ...
          >
            <div className="font-medium text-sm flex items-center gap-2">
              <span>📄</span> Import a spreadsheet
            </div>
            <div className="text-xs text-zinc-600 mt-1">
              Upload CSV/Excel or paste a Google Sheets URL. Skips loading from Teamwork.
            </div>
          </button>
        </div>
